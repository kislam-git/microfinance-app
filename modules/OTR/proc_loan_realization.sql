              ...
              BEGIN 
                ...
                DECLARE ----------------------------------------------- UPDATE BRANCH WISE OTR HISTORY DATA
                    ...
                    SELECT 
                       ...
                       NVL(SUM(ADVANCE_ADJ_PRN),0)ACD_ADV_ADJUST_WSC,
                       NVL(SUM(TOTAL_RECEIVED_WSC),0) ACD_TOTAL_RECEIVED_WSC,
                       ROUND(NVL(SUM(RECEIVABLE_WSC),0) / (NVL(SUM(REG_RCVD_WSC),0) + NVL(SUM(REG_RCVD_WSC),0)),4) * 100  ACD_OTR_WSC,
                       ...
                       ROUND(NVL(SUM(RECEIVABLE_PRN),0) / (NVL(SUM(REG_RCVD_PRN),0) + NVL(SUM(REG_RCVD_PRN),0)),4) * 100  ACD_OTR_PRN
                    FROM  MF_LOAN_REALIZATION
